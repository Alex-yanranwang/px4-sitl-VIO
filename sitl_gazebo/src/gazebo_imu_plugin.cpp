...
void GazeboImuPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  // connect_gazebo_to_ros_topic_pub = node_handle_->Advertise<std_msgs::msgs::ConnectGazeboToRosTopic>(
  //         "~/connect_gazebo_to_ros_subtopic", 1);
  ...
  // //gzmsg<<"119 ~/"<<model_->GetName()<<"XXX"<< imu_topic_<< "\n";
  // connect_gazebo_to_ros_topic_msg.set_ros_topic("/" + model_->GetName() + imu_topic_);
  // connect_gazebo_to_ros_topic_msg.set_msgtype(std_msgs::msgs::ConnectGazeboToRosTopic::IMU);
...
void GazeboImuPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  //gazebo::math::Quaternion Quat_test = gazebo::msgs::Convert(imu_message_.orientation);
  //gzmsg<<"344 imu_message_:"<<C_W_I.X()<< "\n";
  imu_ros_msg_.header.frame_id = frame_id_;
  imu_ros_msg_.header.stamp.sec = current_time.sec;
  imu_ros_msg_.header.stamp.nsec = current_time.nsec;

  imu_ros_msg_.orientation.x = C_W_I.X();
  imu_ros_msg_.orientation.y = C_W_I.Y();
  imu_ros_msg_.orientation.z = C_W_I.Z();
  imu_ros_msg_.orientation.w = C_W_I.W();
  imu_ros_msg_.linear_acceleration.x = linear_acceleration_I[0];
  imu_ros_msg_.linear_acceleration.y = linear_acceleration_I[1];
  imu_ros_msg_.linear_acceleration.z = linear_acceleration_I[2];
