...
void GazeboMotorModel::Publish() {
  ...
  turning_velocity_msg_.set_motor_number(motor_number_);
  //gzmsg <<"36 turning_velocity_msg_:"<<turning_velocity_msg_.data()<<"\n";
  //gzmsg <<"37 GetVelocity:"<<joint_->GetVelocity(0)<<" "<<joint_->GetVelocity(1)<<" "<<joint_->GetVelocity(2)<<"\n";
...
void GazeboMotorModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  // gzmsg << "140 Subscribing to: " << motor_test_sub_topic_ << "\n";
  // motor_sub_ = node_handle_->Subscribe<mav_msgs::msgs::MotorSpeed>("~/" + model_->GetName() + motor_speed_pub_topic_, &GazeboMotorModel::testProto, this);
  ...
  //gzmsg <<"163 Motor_pub_topic:"<<"~/"+model_->GetName() + motor_speed_pub_topic_<<"\n";
  ...
  // Protobuf test
  motor_sub_ = node_handle_->Subscribe<std_msgs::msgs::Float>("~/" + model_->GetName() + motor_speed_pub_topic_, 
                &GazeboMotorModel::testProto, this);
  //gzmsg <<"171 Motor_topic:"<<"~/"+model_->GetName() + motor_speed_pub_topic_<<"\n";
}

// Protobuf test
void GazeboMotorModel::testProto(MotorSpeedPtr &msg) {
  // gzmsg << "166 Received message" << "\n";
  // gzmsg << "167 msg_size: "<<msg->motor_speed_size()<< "\n";
  // for(int i; i < msg->motor_speed_size(); i++){
  //   gzmsg << "169 msg:" << msg->motor_speed(i) <<" ";
  // }
  // gzmsg << "\n";

  //gzmsg <<"179 msg:"<<msg->data()<<"\n";
}
